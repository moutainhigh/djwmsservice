...
//得到一个用来记录日志的对象，这样打印信息的时候能够标记是打印的那个类的信息。
	...
	public Map<String, Object> getOrderByAttributeBO(@RequestBody(required = false) String json,
	...
	public Map<String, Object> getSplitOrderByOrderId(@RequestBody(required = false) String json,
			HttpServletRequest request) {
		try {
			logger.debug("json : " + json);
	...
	public Map<String, Object> getOrderByOrderIdList(@RequestBody(required = false) String json,
			HttpServletRequest request) {
		try {
			logger.debug("json : " + json);
