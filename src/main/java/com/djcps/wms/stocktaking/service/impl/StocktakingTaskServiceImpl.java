...
public class StocktakingTaskServiceImpl implements StocktakingTaskService {
    ...
    public Map<String, Object> getNumber() {
        ...
        if ((map.get(key)).size() > 0) {
            number = AppConstant.TS + (new SimpleDateFormat("yyyyMMdd")).format(new Date())
     ...
     **/
    // public List<StocktakingTaskBO>
    // getPartStocktakingOrderDetail(List<ForderInfoBO> forderInfoBOList,
    // List<LocationOrderInfoBO> locationOrderInfoBOList){
    // List<StocktakingTaskBO> stocktakingTaskBOList=new
    // ArrayList<StocktakingTaskBO>();
    // StocktakingTaskBO stocktakingTaskBO=null;
    // for(LocationOrderInfoBO locationOrderInfoBO:locationOrderInfoBOList){
    // for (ForderInfoBO forderInfoBO : forderInfoBOList){
    // if
    // (locationOrderInfoBO.getOrderId().equals(forderInfoBO.getFchildorderid())){
    ...
    // if(!ObjectUtils.isEmpty(forderInfoBO.getFboxlength()) &&
    // !ObjectUtils.isEmpty(forderInfoBO.getFboxwidth()) &&
    // !ObjectUtils.isEmpty(forderInfoBO.getFboxheight())){
    // forderInfoBO.setFproductRule(new
    // StringBuffer().append(forderInfoBO.getFboxlength()).append("*")
    // .append(forderInfoBO.getFboxwidth()).append("*").append(forderInfoBO.getFboxheight()).toString());
    // }
    // forderInfoBO.setFmaterialRule(new
    // StringBuffer().append(forderInfoBO.getFmateriallength()).append("*")
     ...
     * @date 2018/2/2 17:04
     **/
    public List<StocktakingTaskfBO> getStocktakingOrderDetail(List<ChildOrderBO> childOrderBOList,
            List<LocationOrderInfoBO> locationOrderInfoBOList) {
     ...
     * @date 2018/2/3 10:23
     **/
    // private WarehouseOrderDetailPO getOrderDetail(WarehouseOrderDetailPO source,
    // WarehouseOrderDetailPO target){
    // //规格长宽高都不为null,才进行拼接
    // if(!ObjectUtils.isEmpty(target.getFboxlength()) &&
    // !ObjectUtils.isEmpty(target.getFboxwidth()) &&
    // !ObjectUtils.isEmpty(target.getFboxheight())){
    // //拼接字符串,拼接成产品规格和下料规格
    // source.setFproductRule(new
    // StringBuffer().append(target.getFboxlength()).append("*")
    // .append(target.getFboxwidth()).append("*").append(target.getFboxheight()).toString());
    // }
    // source.setFmaterialRule(new
    // StringBuffer().append(target.getFmateriallength()).append("*")
     ...
     **/
    // private WarehouseOrderDetailPO getOrderDetail(SaveStocktakingOrderInfoBO
    // source, SaveStocktakingOrderInfoBO target){
    // //规格长宽高都不为null,才进行拼接
    // if(!ObjectUtils.isEmpty(target.getBoxLength()) &&
    // !ObjectUtils.isEmpty(target.getBoxWidth()) &&
    // !ObjectUtils.isEmpty(target.getBoxHeight())){
    // //拼接字符串,拼接成产品规格和下料规格
    // source.setProductRule(new
    // StringBuffer().append(target.getBoxLength()).append("*")
    // .append(target.getBoxWidth()).append("*").append(target.getBoxHeight()).toString());
    // }
    // source.setMaterialRule(new
    // StringBuffer().append(target.getMaterialLength()).append("*")
    ...
    public Map<String, Object> stocktakingOrderInfoList(PdaGetStocktakingOrderBO pdaGetStocktakingOrderBO) {
        ...
        for (SaveStocktakingOrderInfoBO saveStock : orderInfoListResult.getTaskOrderInfo().getResult()) {
            ...
            if (!ObjectUtils.isEmpty(saveStock.getBoxLength()) && !ObjectUtils.isEmpty(saveStock.getBoxWidth())
                ...
                saveStock.setProductSize(new StringBuffer().append(saveStock.getBoxLength()).append("*")
                        .append(saveStock.getBoxWidth()).append("*").append(saveStock.getBoxHeight()).toString());
            }
            if (!ObjectUtils.isEmpty(saveStock.getMaterialLength())
                    && !ObjectUtils.isEmpty(saveStock.getMaterialWidth())) {
    ...
    public Map<String, Object> pdaInventorySurplus(StocktakingTaskBO stocktakingTaskBO) {
        ...
        if (!ObjectUtils.isEmpty(httpResult.getData())) {
            BatchOrderDetailListPO batchOrderDetailListPO = dataFormatGson.fromJson(gson.toJson(httpResult.getData()),
        ...
        // HttpResult
        // warehouseAreaAndLocResult=stocktakingTaskServer.pdaWarehouseAreaAndLocInfo(stocktakingTaskBO);
        return MsgTemplate.successMsg(stocktakingTaskBO);
    }
     ...
     **/
    @Override
    public Map<String, Object> saveStocktakingResult(SaveStocktakingOrderInfoList saveStocktakingOrderInfoList,
            PartnerInfoBO partnerInfoBO) {
        List<String> orderidlist = new ArrayList<String>();
        ...
        for (JsonElement jsonElement : jsonArray) {
            SaveStocktakingOrderInfoBO saveStocktakingOrderInfoBO1 = gson.fromJson(jsonElement,
                    SaveStocktakingOrderInfoBO.class);
        ...
        for (SaveStocktakingOrderInfoBO foresave : saveStocktakingOrderInfoList.getSaveStocktaking()) {
            ...
            for (SaveStocktakingOrderInfoBO backgroundsave : list) {
                ...
                if (foresave.getIsInventoryProfit().equals(2)) {
                    if (foresave.getOrderId().equals(backgroundsave.getOrderId())
                            && foresave.getWarehouseAreaId().equals(backgroundsave.getWarehouseAreaId())
                            && foresave.getWarehouseLocId().equals(backgroundsave.getWarehouseLocId())) {
                        // 保存盘点数量
                        ...
                        BeanUtils.copyProperties(backgroundsave, foresave,
                                new String[] { "takeStockAmount", "differenceValue" });
                        if (ObjectUtils.isEmpty(foresave.getTakeStockAmount())) {
        ...
        List<ChildOrderBO> childOrderList = orderServer.getChildOrderList(orderIdsBO);
        for (SaveStocktakingOrderInfoBO saveStocktakingOrderInfoBO3 : saveStocktakingOrderInfoList
                .getSaveStocktaking()) {
            if (saveStocktakingOrderInfoBO3.getIsInventoryProfit().equals(1)) {
                Optional optional = childOrderList.stream()
                        .filter(b -> b.getChildOrderId().equals(saveStocktakingOrderInfoBO3.getOrderId())).findFirst();
                if (optional.isPresent()) {
            ...
            if (!ObjectUtils.isEmpty(joinOrderParamInfo)) {
                ...
                for (WarehouseOrderDetailPO orderInfoList : joinOrderParamInfo) {
                    if (info.getOrderId().equals(orderInfoList.getOrderId())) {
                        // 计算操作面积
                        double area = operationRecordServer.getVolume(
                                Double.parseDouble(orderInfoList.getMaterialLength()),
                                Double.parseDouble(orderInfoList.getMaterialWidth()), info.getTakeStockAmount());
     ...
     * @param param
     */
    public List<OrderOperationRecordPO> orderStocktakingOperationInfo(SaveStocktakingOrderInfoList param,
            PartnerInfoBO partnerInfoBO) {
     ...
     **/
    @Override
    public Map<String, Object> completeStocktakingTask(SaveStocktakingOrderInfoList saveStocktakingOrderInfoBOList,
            PartnerInfoBO partnerInfoBO) {
        // 盘点订单操作记录数据组合
        List<OrderOperationRecordPO> orderOperationInfo = orderStocktakingOperationInfo(saveStocktakingOrderInfoBOList,
                partnerInfoBO);
        ...
        // 校验是否盘点完毕
        for (SaveStocktakingOrderInfoBO saveStocktakingOrderInfoBO1 : saveStocktakingOrderInfoBOList
                .getSaveStocktaking()) {
        ...
        // 赋值盘盈的订单
        for (SaveStocktakingOrderInfoBO saveStocktakingOrderInfoBO3 : saveStocktakingOrderInfoBOList
                .getSaveStocktaking()) {
            if (saveStocktakingOrderInfoBO3.getIsInventoryProfit().equals(1)) {
                Optional optional = childOrderList.stream()
                        .filter(b -> b.getChildOrderId().equals(saveStocktakingOrderInfoBO3.getOrderId())).findFirst();
        ...
        List<SaveStocktakingOrderInfoBO> collectList = saveStocktakingOrderInfoBOList.getSaveStocktaking().stream()
                .filter(b -> b.getDifferenceValue() != 0).collect(Collectors.toList());
        ...
        if (saveresult.isSuccess()) {
            if (!ObjectUtils.isEmpty(orderIdList)) {
                // 修改oms异常订单的标记
                HttpResult addAbnormalResult = abnormalServer.updateExecptionFlag(1, orderIdList,
                        partnerInfoBO.getPartnerArea());
    ...
    public Map<String, Object> pdaStocktakingOrderList(PdaGetStocktakingOrderBO pdaGetStocktakingOrderBO) {
        ...
        for (JsonElement jsonElement : asJsonArray) {
            ...
            pdaStocktakingOrderBO.setMaterialSize(new StringBuffer().append(pdaStocktakingOrderBO.getMaterialLength())
                    .append("*").append(pdaStocktakingOrderBO.getMaterialWidth()).toString());
        ...
        for (PdaStocktakingOrderBO pdaStocktakingOrderBO : pdaorderlist) {
            if (StocktakingTaskConstant.INVENTORY_IS.equals(pdaStocktakingOrderBO.getIsInventoryProfit())
                    && pdaStocktakingOrderBO.getStatus().equals(StocktakingTaskConstant.STATUS_3)) {
                pdaorderlist2.add(pdaStocktakingOrderBO);
            }
        }
        for (PdaStocktakingOrderBO pdaStocktakingOrderBO : pdaorderlist) {
            if (StocktakingTaskConstant.INVENTORY_NORMAL.equals(pdaStocktakingOrderBO.getIsInventoryProfit())
                    && pdaStocktakingOrderBO.getStatus().equals(StocktakingTaskConstant.STATUS_3)) {
                pdaorderlist2.add(pdaStocktakingOrderBO);
            }
        }
    ...
    public List<OrderOperationRecordPO> pdaOrderStocktakingOperationInfo(SaveStocktakingOrderInfoBO param) {
        ...
        // 计算操作面积
        double area = operationRecordServer.getVolume(Double.parseDouble(param.getMaterialLength()),
                Double.parseDouble(param.getMaterialWidth()), param.getTakeStockAmount());
        orderOperationRecordPO.setArea(String.valueOf(area));
        list.add(orderOperationRecordPO);
        return list;
    }
    ...
    public Map<String, Object> savePdaStocktakingResult(SaveStocktakingOrderInfoBO saveStocktakingOrderInfoBO) {
        ...
        saveStocktakingOrderInfoBO
                .setFluteType(StringUtils.switchFluteTypeToNumber(saveStocktakingOrderInfoBO.getFluteType()));
        List<String> orderidlist = new ArrayList<String>();
        // 获取所有订单列表
        orderidlist.add(saveStocktakingOrderInfoBO.getOrderId());
        ...
        if (saveStocktakingOrderInfoBO.getIsAdd().equals(StocktakingTaskConstant.ISADD_NEW)) {
            ...
            if (!ObjectUtils.isEmpty(pdaStocktakingOrder.getData())) {
                JsonObject pdaOrderJsonArray = new JsonParser().parse(gson.toJson(pdaStocktakingOrder.getData()))
                        .getAsJsonObject();
                ...
                for (WarehouseAreaAndLocBO warehouseAreaAndLocBO : pdaOderInfoBO.getOrderAreaAndLocList()) {
                    if (saveStocktakingOrderInfoBO.getWarehouseAreaId()
                            .equals(warehouseAreaAndLocBO.getWarehouseAreaId())
                            && saveStocktakingOrderInfoBO.getWarehouseLocId()
                                    .equals(warehouseAreaAndLocBO.getWarehouseLocId())) {
                        // 该新增订单已存在
    ...
    public Map<String, Object> stocktakingCompleteStatus(GetStocktakingTaskBO getStocktakingTaskBO) {
        ...
        for (int i = 0; i < completeStatusPOList.size(); i++) {
            ...
            StocktakingCompleteStatusPO stocktakingCompleteStatusPO = gson.fromJson(complete,
                    StocktakingCompleteStatusPO.class);
