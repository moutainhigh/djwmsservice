...
public class OrderServer {
    ...
    public HttpResult getOrderByOrderId(OrderIdBO param) {
        ...
        okhttp3.RequestBody rb = okhttp3.RequestBody.create(okhttp3.MediaType.parse("application/json; charset=utf-8"),
                json);
        // 调用借口获取信息
        ...
        for (UpdateOrderBO updateOrderBO : param) {
            ...
            if (orderId.indexOf(OrderConstant.ONLINE_PAPERBOARD_ORDER) != -1
                ...
                onlineList.add(updateOrderBO);
                    ...
                    || orderId.indexOf(OrderConstant.OFFLINE_BOX_ORDER) != -1) {
    ...
    public Boolean compareOrderStatus(List<String> orderIdList, String partnerArea, String partnerId) {
        ...
        if (!ObjectUtils.isEmpty(orderIdList)) {
            ...
            BatchOrderDetailListPO batchOrderDetailListPO = dataFormatGson
                    .fromJson(gsonNotNull.toJson(orderDeatilByIdList.getData()), BatchOrderDetailListPO.class);
            ...
            Map<String, List<WarehouseOrderDetailPO>> detailMap = dataFormatGson.fromJson(
                    gsonNotNull.toJson(splitOrderResult.getData()),
                    new TypeToken<Map<String, List<WarehouseOrderDetailPO>>>() {
                    }.getType());
            for (Map.Entry<String, List<WarehouseOrderDetailPO>> entry : detailMap.entrySet()) {
                ...
                if (!ObjectUtils.isEmpty(detailValueList)) {
                    for (WarehouseOrderDetailPO orderDetailPO : detailValueList) {
                        ...
                        if (splitOrderStatus.equals(OrderStatusTypeEnum.NO_STOCK)
                                || splitOrderStatus.equals(OrderStatusTypeEnum.LESS_ADD_STOCK)
                                || splitOrderStatus.equals(OrderStatusTypeEnum.ALL_ADD_STOCK)
                                || splitOrderStatus.equals(OrderStatusTypeEnum.ORDER_ALREADY_ALLOCATION)
                                || splitOrderStatus.equals(OrderStatusTypeEnum.ORDER_ALREADY_DELIVERY)
                                || splitOrderStatus.equals(OrderStatusTypeEnum.ORDER_ALREADY_LOADING)
                            ...
                            if (Integer.valueOf(splitOrderStatus).intValue() < orderStatus.intValue()) {
                                // 一个开关代表需要进行修改
                                flag = true;
                                orderDetailMap.get(orderDetailPO.getOrderId())
                                        .setOrderStatus(orderDetailPO.getSubStatus());
    ...
    public List<WarehouseOrderDetailPO> joinOrderParamInfo(List<WarehouseOrderDetailPO> param) {
        for (WarehouseOrderDetailPO onlinePaperboardPO : param) {
            // 规格长宽高都不为null,才进行拼接
            if (!ObjectUtils.isEmpty(onlinePaperboardPO.getBoxLength())
                    && !ObjectUtils.isEmpty(onlinePaperboardPO.getBoxWidth())
                    && !ObjectUtils.isEmpty(onlinePaperboardPO.getBoxHeight())) {
                onlinePaperboardPO.setProductSize(new StringBuffer().append(onlinePaperboardPO.getBoxLength())
                        .append("*").append(onlinePaperboardPO.getBoxWidth()).append("*")
                        .append(onlinePaperboardPO.getBoxHeight()).toString());
            }
            if (!ObjectUtils.isEmpty(onlinePaperboardPO.getMaterialLength())
                    && !ObjectUtils.isEmpty(onlinePaperboardPO.getMaterialWidth())) {
                onlinePaperboardPO.setMaterialSize(new StringBuffer().append(onlinePaperboardPO.getMaterialLength())
                        .append("*").append(onlinePaperboardPO.getMaterialWidth()).toString());
            }
            onlinePaperboardPO.setAddressDetailProvince(new StringBuffer().append(onlinePaperboardPO.getCodeProvince())
                    .append(onlinePaperboardPO.getAddressDetail()).toString());
    ...
    public List<WarehouseOrderDetailPO> getOrderStockInfo(SelectAreaByOrderIdBO param) {
        ...
        if (!ObjectUtils.isEmpty(stockInfoResult.getData())) {
            List<WarehouseOrderDetailPO> orderStockInfoList = gson.fromJson(gson.toJson(stockInfoResult.getData()),
                    new TypeToken<ArrayList<WarehouseOrderDetailPO>>() {
                    }.getType());
    ...
    public BatchOrderDetailListPO getOrderOrSplitOrder(OrderIdsBO param) {
        ...
        if (!ObjectUtils.isEmpty(httpResult.getData())) {
            batchOrderDetailListPO = dataFormatGson.fromJson(gsonNotNull.toJson(httpResult.getData()),
    ...
    public List<ChildOrderBO> getChildOrderList(OrderIdsBO param) {
        ...
        if (!ObjectUtils.isEmpty(httpResult.getData())) {
            ...
            // List<WarehouseOrderDetailPO> orderList =
            // batchOrderDetailListPO.getOrderList();
            List<WarehouseOrderDetailPO> joinOrderParamInfo = joinOrderParamInfo(orderList);
            for (WarehouseOrderDetailPO warehouseOrderDetailPO : joinOrderParamInfo) {
